...
parser = argparse.ArgumentParser(description='Implement DPFPS by PyTorch')
...
def main():
    ...
    args.save_time = args.save_time + '_spsr_sigmoid_filter_sum_context_add_scratch_' + time.strftime("%Y%m%d%H%M", time.localtime())
    ...
    log2 = open(os.path.join(args.save_dir, '{}_sparsity_epoch.txt'.format(args.save_time)), 'w')
    log3 = open(os.path.join(args.save_dir, '{}_sparsity_allocation_epoch.txt'.format(args.save_time)), 'w')
    ...
    method = DPFPS(model, args.lambda21, args.pr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        model_pruning = copy.deepcopy(model)
        method1 = DPFPS(model_pruning, args.lambda21, args.pr)
        ...
        method2 = DPFPS(model_pruning1, args.lambda21, args.pr)
        method2.adjust_scale_coe(next_ratio)
        method2.channel_prune()
        print(next_ratio)
        method.layer_sparsity_allocation_ratio = method2.layer_sparsity_allocation_ratio
    ...
    if args.cuda:
        model.cuda()
    method2 = DPFPS(model, args.lambda21, args.pr)
    method2.channel_prune()
    model = method2.model
    if args.cuda:
        model.cuda()
...
class DPFPS:
    ...
    def switch_row_sparsity(self):
        self.column_flag = False

    def get_update_index(self, conv_layer, next_conv_layer, sar_num):
        ...
        # num = min(int(conv_layer.out_channels * sar_num), conv_layer.out_channels)
        num = min(math.ceil(conv_layer.out_channels * sar_num), conv_layer.out_channels)
        # void zero
        num = max(num, 1)
        ...
        # print(mul_t_t1)
        # print(add_t_t1)
        _, index = torch.topk(add_t_t1, num, largest=False)
        ...
        if 'vgg' in args.arch:
            for i, layer_index in enumerate(self.list1):
                if self.lambda_L21 > 1e-6:
                    if i < len(self.list1) - 1:
                        update_index = self.get_update_index(self.model.module._modules['feature'][layer_index], self.model.module._modules['feature'][self.list1[i + 1]], self.layer_sparsity_allocation_ratio[m])[0]
                        ...
                        update_index = self.get_update_index(self.model.module._modules['feature'][layer_index], self.model.module._modules['classifier1'], self.layer_sparsity_allocation_ratio[m])[0]
                        self.model.module._modules['feature'][layer_index].weight.data = self.L21_regularizer(self.model.module._modules['feature'][layer_index], update_index)
            ...
            for layer_index, layer_index_value in enumerate(self.list1):
                for j in range(self.model.module.layernum[layer_index]):
                    if self.lambda_L21 > 1e-6:
                        update_index = self.get_update_index(self.model.module._modules[layer_index_value][j].conv1, self.model.module._modules[layer_index_value][j].conv2, self.layer_sparsity_allocation_ratio[m])[0]
                    ...
                    if args.arch in ['resnet50', 'resnet101']:
                        if self.lambda_L21 > 1e-6:
                            update_index = self.get_update_index(self.model.module._modules[layer_index_value][j].conv2, self.model.module._modules[layer_index_value][j].conv3, self.layer_sparsity_allocation_ratio[m])[0]
    ...
    def row_prune(self, layer_p, BN_layer_p, layer1_p):
