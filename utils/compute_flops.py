...
# __all__ = ['print_model_param_nums', 'count_model_param_flops']
...
def count_model_param_flops(model=None, input_res=224, multiply_adds=True):
    ...
    def conv_hook(self, input, output):
        ...
        kernel_ops = self.kernel_size[0] * self.kernel_size[1] * int(self.in_channels / self.groups)
    ...
    def foo(net):
        ...
        if not childrens:
            if isinstance(net, torch.nn.Conv2d):
                hand.append(net.register_forward_hook(conv_hook))
    ...
    foo(model)
    # print(hand)
    input = torch.FloatTensor(1, 3, input_res, input_res).cuda()
    _ = model(input)
    for h in hand:
        h.remove()
